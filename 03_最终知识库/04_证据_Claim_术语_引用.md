...
**判断与证据的阅读距离：** 需要 evidence 才能成立的核心判断，应让 reviewer 在同一局部论证链中找到对应的直接依据、结果读取或准确 locator，无需跨过无关内容反查；不能先连续堆出多个结论，再让读者到远处寻找各自依据。判断可以先提出，也可以在必要的设置和观察之后得出，次序由 `01`–`03` 的 TIE 功能推进决定。这里检查的是判断与依据能否就近对应，不另规定固定句序或图文格式。
...
| Nature/ARS | 仅提供知识组织、分阶段执行、局部修订及科学表达、证据/claim/source/consistency 的核查方法 | 项目事实，或 Response 的 TIE 文风、功能顺序与具体句式 |
