...
上表只提供常见功能索引，不穷尽所有必要句子。若一句无法归入表中类型，先依据当前 concern、证据和上下文判断其真实科学或沟通职责；不能仅因未匹配类别就删除，或为匹配分类而改写必要内容。确认其没有独立必要功能后，再按 §4 的必要性检查决定删除、合并或保留。
...
**符号可读性：** 关键量首次进入 reviewer-facing prose 时，给出来自当前项目材料的清楚技术名称与正式符号；之后保持同一名称与符号的对应。逐句判断 reviewer 在当前阅读位置能否直接读出技术含义，不能以“前文定义过、符号一致或含义可恢复”代替可读性检查。在新的 Paragraph 或 evidence block、关键 criterion、主要结果读取、comparison、claim 和 conclusion 等需直接判断含义的位置，按阅读需要保留技术含义，可用“名称＋符号”或直接的文字关系，不让连续表述退化为裸符号解码。紧邻重复、局部公式推导或唯一且清楚的回指可简化，不要求每次重写全称。TIE 的技术名词连续复现支持可读性选择，具体名称与符号仍由当前项目证据决定。
...
**范围词核对：** 在比较和结论句中，`across/all/selected/generally` 等词会改变覆盖范围或判断强度。按 `04` 的 SD-01 核对其所限定的对象、设置、baseline、metric 和具体比较关系，是否对应实际 evidence range；语法自然不等于范围正确。相邻句判断不同关系时，覆盖集合可以不同，不能仅因范围词变化判矛盾；应核对各句集合是否明确，以及设置、比较动作和比较结果是否被准确命名。范围词依当前证据判断，不构成固定禁词表。
...
- **语言与科学关系联合判定。** `02` 检查语言实现，`04` 和当前项目 primary evidence 核对科学关系。按 `06 §5.2` 在同一句内先核对实际含义，再专读表达，最后合并判断；联合正确不要求一次同时处理全部判据。关键短语包括影响自然性、对象命名和可读性的表达，不只包括改变科学含义的短语。事实待核实不妨碍发现当前文字中的表达问题；语言自然也不能替代科学附着核验。
- **区分观察与修正。** SL 中标为 TIE 稳定倾向的内容可作为表达指纹；为修复原文语法、搭配、指代或过强 claim 而采取的自然化处理，仅保证专业正确性，不得宣称为 TIE 的固定写法。

## 4. 句法级 TIE-style audit

保留 reviewer situation、段落任务及相邻句语境，每次只处理一句，按 `06 §5.2` 作两次聚焦阅读，只调用当前句实际需要的 SL 判据：

1. **先读实际含义。** 本句要推进什么判断或动作？谁与什么对象发生何种关系？条件、方式、baseline、metric 和范围分别修饰什么？用 `04` 核对它们与项目事实的对应，区分已可判断的文字关系与待核实的事实。
2. **再专读表达。** 主语、具体动词和时态是否承担本句责任；名词组合、搭配和信息顺序是否自然、直接地表达对象、动作或结果？技术含义是否在当前阅读位置可读，指代是否唯一，连接是否回应相邻句的真实关系？事实或指向可恢复，不能替代这些表达判断。

对压缩组合和元叙述式名词组，实际检查其命名和搭配，不因“不改变科学事实”就跳过，也不因较概括就判错。遇到功能或措辞争议，按需要回看同功能的 TIE 句段，核对对象、动作和推进，不以相同前缀、语态或某短语未在 TIE 出现作为裁决标准。TIE 的语法、搭配、冗余或过强 claim 缺陷仍只作最小专业化修正，不作为表达指纹。

完成两次阅读后合并判断：**sentence-level PASS = 语言实现正确且科学附着正确**。事实未核实不能冒充整句通过，也不能取代对可观察表达问题的判断。记录应能定位实际短语并说明依据；列齐句子编号或声明“已检查”不证明判据已经落实。只有确认句子没有必要的回答、解释、证据、限定、承接或定位功能，且删除或合并不损害清晰度、准确性或可追溯性时，才删去或并入邻句。
