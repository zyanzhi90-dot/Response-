...
阶段 3B 在 TN-06 内增加 **11 个 sentence-level language units（SL-01 至 SL-11）**。它们是同功能 TIE realization 比较的辅助判据，不单独构成“英语是否可接受”的主检查表，也不改变 `01` 已确定的信息顺序和证据链。下文出现的方括号结构是句法槽位，不是可直接复制的模板。
...
先按 `01` 判定 B1–B6 的功能块，确认当前句群下一步需要 action、局部解释、推导、setup、locator 还是 observation；Sentence/Phrase 按 §4 调用同功能 TIE 证据独立形成 realization，本表只把 sentence intent 路由到相关 SL-01～SL-11 辅助判据，不构成第二套 response organization 规则。
...
**Sentence-level PASS** 要求可判断的语言实现与科学附着均无真实 blocker；未核实事实保持 unverified，不因无法证明 PASS 就直接判 FAIL，也不妨碍指出可观察的表达问题。记录应定位实际句子或短语及差异依据；列齐句子编号或声明“已检查”不证明比较已执行。只有确认句子没有必要的回答、解释、证据、限定、承接或定位功能，且删除或合并不损害清晰度、准确性或可追溯性时，才删去或并入邻句。
