...
在进入 Paragraph 前，依 `01` 与 TIE realization 基准 §2 的同功能 A/B 机制，比较整条主线和 Part 次序：实质答案或动作是否及时出现，每次转入解释、证据、比较或修改是否由上一判断的真实需求引出，是否有跳步、重复启动或过长铺垫。这里校准的是功能推进与节奏；作者已确认路线和当前 evidence 决定具体内容，不能照搬范例的完整块序或篇幅。
...
准备新增 Paragraph 时，按 `01` TN-02 判断它是否需要独立的条件、证据或读取过程；现段可自然承接的短动作或补充就地替换、合并或压缩，确需新的段内推进时才扩展。
...
完成当前 Part 后，从整体职责回看：各 Paragraph 是否共同闭合该 Part 的判断；证据是否集中而不重复；段间是否沿 `01` 与 TIE 基准 §3–4 的同功能状态自然推进。若有下一 Part，确认它确有新的回答职责，并让入口接住前一 Part 留下的理解需求、迅速落到新对象或动作，而非只靠连接词或重复旧结论过渡。通过后才在记录中关闭该 Part；若失败，回到根因及受影响内容，不默认重写整个 Response。
...
2. Whole Response 的核心判断链、Part 顺序及段间推进是否符合当前认知依赖、作者已确认路线与适用的 TIE 同功能推进机制；
...
通过稳定章节定位加载知识：`01` 负责组织，`02` 负责连续句与语言实现，`03` 负责实验和图文，`04` 负责 evidence/claim/数学/术语/source，`05` 负责错误模式，`07` 负责条件规则。`04_TIE句子级表达重建/00_TIE_2621_sentence_realization基准.md` 的 §2 用于 Whole Response/Part 的同功能顺序比较，§3–4 用于逐段连续推进，§4–5 用于 Sentence/Phrase realization；只取当前判断所需的部分。`08` 与该目录其余材料主要在解释历史裁决、C 类边界、证据冲突或可疑规则时溯源，均不构成第二套运行流程。
