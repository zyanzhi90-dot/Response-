...
若能准确概括，先用一句内部短句说清这条 Response 最重要、可由证据支持的直接回答，再据此检验各核心判断是否服务该回答。若有多个必须分别闭合的 obligations，就保留最短的有序判断，不把它们强压成单一论点。这个锚点不是 reviewer-facing 的固定开头，也不能覆盖作者已确认的回应路线。
...
对每个 Paragraph 顺读全部句子，结合 `01` 与 TIE realization 基准 §3–4 的同功能句段，检查实际成文是否按当前 concern 的认知需求完成状态转换：每句建立了什么、下一句为何接它、科学对象如何延续或切换、证据如何进入并被读出。不能用连接词掩盖跳步，也不能以逐句局部 PASS 代替整段的 TIE 推进比较。
...
- **Draft**：先完成当前层的可用规划；逐句重建所需 meaning，按同功能 TIE 证据形成 shadow realization，再写当前 Paragraph，并比较成句与独立 realization；完成连续句核对后进入下一段。
- **Audit**：把现有 Paragraph 的实际功能和推进顺序与上层规划逐项比较；对全部句子执行 meaning reconstruction → shadow realization → differential comparison，记录有依据的真实问题，不修改文本。
- **Revise**：先确认问题的根因层级及受影响范围；必要时更新上层规划，对受影响句重新执行同一机制，只修改需要修复的内容并复核依赖链。
