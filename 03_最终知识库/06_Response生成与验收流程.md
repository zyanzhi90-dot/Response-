...
在该段及相邻句语境中，每次只处理一句，按 `02 §4` 执行 **meaning reconstruction → 独立 TIE shadow realization → differential comparison**；用 `04` 核对科学关系、证据范围和语言强度。比较每个承担实质意义的词组如何命名对象、分配主语和动作、附着限定、安排信息、延续指代和保持符号可读性，并核对这句话是否完成当前功能。`02` 的 SL 单元只辅助判断；语法正确或意思可恢复不能代替同功能 TIE 比较。事实待核实时保留该状态，同时完成仍可观察的表达审查。
