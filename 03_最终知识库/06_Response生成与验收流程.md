...
记录与任务范围相称，绑定实际处理的文本和材料版本，保留当前判断所需的 reviewer 关切、上层主线、依据、未决事实和受影响范围。局部表达审计不为未处理的层级另建完整 map、规则清单或交付状态。
...
通过稳定章节定位加载知识：`01` 负责组织，`02` 负责连续句与语言实现，`03` 负责实验和图文，`04` 负责 evidence/claim/数学/术语/source，`05` 负责错误模式，`07` 负责条件规则。`04_TIE句子级表达重建/00_TIE_2621_sentence_realization基准.md` 是 Sentence/Phrase Draft、Audit、Revise 的默认 realization 证据；`08` 与该目录其余材料主要在解释历史裁决、C 类边界、证据冲突或可疑规则时溯源，均不构成第二套运行流程。
